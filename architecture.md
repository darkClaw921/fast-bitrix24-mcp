...
- `fast_bitrix24_mcp/tools/deal.py`
  ...
  - Инструменты:
    ...
    - `get_deals_at_risk(filter_fields: dict[str, str] = {}, fields_id: list[str] = ["ID", "TITLE", "STAGE_ID", "DATE_MODIFY"])` — получение сделок, находящихся в риске. **Критерии риска**: сделка считается «в риске», если выполняется одно или несколько условий: статус сделки не менялся более 5 рабочих дней (исключая субботу и воскресенье); отсутствует активность (звонки, комментарии, задачи) более 3 дней. **Проверка активности**: для каждой сделки проверяется наличие звонков через `get_crm_activities_by_filter` (фильтр по `ENTITY_TYPE='DEAL'` и `ENTITY_ID`), комментариев через `crm.timeline.comment.list`, задач через `get_tasks_by_filter` с проверкой поля `UF_CRM_TASK` в формате `["D_{deal_id}"]`. Возвращает текстовый отчет со списком сделок в риске с указанием причин риска и статистики активности за последние 3 дня. **Вспомогательные функции**: `_count_workdays(start_date, end_date)` — подсчет рабочих дней между двумя датами (исключая выходные); `_get_deal_activity(deal_id, days=3)` — получение активности по сделке за указанный период с возвратом информации о звонках, комментариях, задачах и дате последней активности.
  ...
  - Вспомогательные функции:
    ...
    - `_get_deal_activity(deal_id: int, days: int = 3)` — получение активности по сделке за указанный период. Проверяет наличие звонков (через `get_crm_activities_by_filter`), комментариев (через `crm.timeline.comment.list`) и задач (через `get_tasks_by_filter` с проверкой поля `UF_CRM_TASK`). Возвращает словарь с информацией о наличии активности, дате последней активности и количестве активностей каждого типа. Используется для проверки критерия риска "отсутствует активность более 3 дней".
