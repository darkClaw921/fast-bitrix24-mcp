...
- `fast_bitrix24_mcp/tools/helper.py`
  ...
  - Вспомогательные функции для работы с датами:
    ...
    - `_apply_condition_for_task(records, condition)` — применение условия к записям задач с поддержкой преобразования форматов полей. **Особенность**: использует `_compare` для сравнения значений, что обеспечивает нормализацию типов (строка `"123"` сравнивается с числом `123` как равные).
